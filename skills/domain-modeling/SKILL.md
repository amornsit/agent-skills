...
Multi-context repo (presence of `CONTEXT-MAP.md` at the root). The map points to where each context lives:
...
Create files lazily — only when you have something to write. If no `CONTEXT.md` exists, create one when the first term is resolved. If no `docs/adr/` exists, create it when the first ADR is needed. Never scaffold either ahead of need, and never flag their absence: other skills read these docs and are told to proceed silently when they're missing.
