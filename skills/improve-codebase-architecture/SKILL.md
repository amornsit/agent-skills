...
- Read [codebase-design](../codebase-design/SKILL.md) for the architecture vocabulary (**module**,
...
See [references/html-report.md](references/html-report.md) for the full HTML scaffold, diagram
...
Once the user picks a candidate, run [grilling](../grilling/SKILL.md) to walk the decision tree with
them — constraints, dependencies, the shape of the deepened module, what sits behind the seam, what
tests survive.

Side effects happen inline as decisions crystallize — run
[domain-modeling](../domain-modeling/SKILL.md) to keep the domain model current as you go:
...
_Adapted from Matt Pocock's "improve-codebase-architecture" skill (github.com/mattpocock/skills) —
MIT © Matt Pocock.
`HTML-REPORT.md` moved to `references/html-report.md` with the context pointer repointed, and the
`/codebase-design`, `/grilling`, and `/domain-modeling` slash commands turned into cross-skill links.
