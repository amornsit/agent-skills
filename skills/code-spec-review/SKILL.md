...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the workflow in `docs/agents/issue-tracker.md`. If no tracker has been configured, default to the local-markdown tracker: the spec is `.scratch/<feature-slug>/spec.md` and issues are `.scratch/<feature-slug>/issues/NN-<slug>.md`. Run `/setup-skills` to point this repo at a different tracker.
...
Feature Envy, Shotgun Surgery, Divergent Change, and Middle Man are all really questions about where a module's boundary sits; [`codebase-design`](../codebase-design/SKILL.md) carries the shared vocabulary for naming that precisely.
